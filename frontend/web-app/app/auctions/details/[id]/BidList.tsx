    ...
    return (
        <div className='rounded-lg shadow-md'>
            <div className='py-2 px-4 bg-white'>
                <div className='sticky bg-white p-2'>
                    <Heading title={`Current High Bid is $${numberWithCommas(highBid)}`}/>
                </div>
            </div>

            <div className='overflow-auton h-[400px] flex flex-col px-2'>
                {bids.length === 0 ? (
                    <EmptyFilter title='No Bids for this item' subtitle='Please feel free to make a Bid' ></EmptyFilter>
                ) : (
                    <>
                        {bids.map(bid => (
